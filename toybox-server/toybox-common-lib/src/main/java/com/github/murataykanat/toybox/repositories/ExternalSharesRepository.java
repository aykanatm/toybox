...
public interface ExternalSharesRepository extends JpaRepository<ExternalShare, String> {
    @Query(value = "SELECT external_share_id, username, job_id, expiration_date, max_number_of_hits, notify_when_downloaded, enable_expire_external, enable_usage_limit FROM external_shares WHERE external_share_id=?1", nativeQuery = true)
    List<ExternalShare> getExternalSharesById(String externalShareId);

    @Transactional
    @Modifying
    @Query(value = "INSERT INTO external_shares(external_share_id, username, job_id, expiration_date, max_number_of_hits, notify_when_downloaded, enable_expire_external, enable_usage_limit) VALUES (:external_share_id, :username, :job_id, :expiration_date, :max_number_of_hits, :notify_when_downloaded, :enable_expire_external, :enable_usage_limit)", nativeQuery = true)
    int insertExternalShare(@Param("external_share_id") String externalShareId, @Param("username") String username, @Param("job_id") long jobId,
                            ...
                            @Param("notify_when_downloaded") String notifyWhenDownloaded, @Param("enable_expire_external") String enable_expire_external,
                            @Param("enable_usage_limit") String enable_usage_limit);
